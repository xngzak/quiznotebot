...
def extract_needed_data(page):
    ...
    if title_prop.get("type") == "title":
        ...
        title = "".join([text.get("text", {}).get("content", "") for text in title_texts])
...
class TwitterAPIv2:
    ...
    def upload_media(self, image_path):
        ...
        try:
            ...
            # media ファイル
            form_data.append(f'--{boundary}')
            form_data.append('Content-Disposition: form-data; name="media"; filename="image.jpg"')
            ...
            # 完全なボディを作成
            body_bytes = text_part.encode('utf-8') + image_data + end_boundary.encode('utf-8')
            ...
            conn.request('POST', '/1.1/media/upload.json', body=body_bytes, headers=headers)
            response = conn.getresponse()
            append_response = response.read().decode('utf-8')
            conn.close()
            
            if response.status != 204:  # APPENDは204 No Contentを返す
                print(f"❌ APPEND failed: Status {response.status}, Response: {append_response}")
                return None
            ...
            auth_header = self._generate_oauth_header('POST', finalize_url, finalize_params)
